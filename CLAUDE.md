...
- **PII 类报告（默认存入 `portfolio/reports/`，不进公开仓库）**：买入价分析（`/entry-price`，`*-entry-*.md`）、多公司持仓买入价对比、开仓/加仓价位复核——凡包含账户信息、逐笔持仓股数/成本价、或个人开仓/加仓/减仓触发价的报告，一律写入 `portfolio/reports/{公司名}/`（跨公司/复核类放 `portfolio/reports/` 根目录），目录结构镜像公开仓库的 `reports/`
- **纯研究报告仍留公开仓库**：`/investment-research`、`/industry-research`、`/earnings-review` 等仅含生意/护城河/估值/行业分析、不含个人持仓数据的报告，仍按"公开决策、不公开规模"原则放公开 `reports/`
- PII 报告若引用公开仓库中的研究/skill，用文件系统相对路径（`portfolio/` 嵌套于主仓库内，如 `../../reports/…`、`../../../reports/…`、`../../skills/…`）以便本地跳转
...
| /entry-price | `{公司名}-entry-{YYYYMMDD}.md`（PII，私有子模块） | `portfolio/reports/Tesla/特斯拉-entry-20260708.md` |
